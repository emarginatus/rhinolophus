...
#' @param delta.amplitude The step size in which the pulse is extracted. See details
#' @examples
#'  wav <- read_wav(
        ...
        pulses <- do.call(
          ...
          lapply(
            ...
            function(zone){
              ...
              for (step in seq_len(n.step)) {
                to.low <- which(db < (min.amplitude + step * delta.amplitude))
                if (length(to.low) > 0) {
                  db <- db[-to.low]
                ...
                while (i <= length(candidate)) {
                  minimum.contour <- clump(
                    working >= (db[i] - step * delta.amplitude),
                    directions = 4
                  )
