...
#' @param spectrogram A spectrogram as generated by \code{\link{wav.2.spectrogram}}
#' @param contour.level A vector of contour levels. If one number it is used as the step size in the vector.
...
#' @export
spectrogram.contour <- function(spectrogram, contour.level = NULL){
  if(missing(contour.level)){
    contour.level <- seq(0, ceiling(max(spectrogram$S)), by = 1)
  } else {
    if(length(contour.level) == 1){
      contour.level <- seq(0, contour.level * (max(spectrogram$S) %/% contour.level), by = contour.level)
    }
  }
  ...
  contour.line$level <- as.numeric(levels(contour.line$level))[contour.line$level]
